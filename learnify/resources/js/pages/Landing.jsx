    ...
    <div>
      ...
      <section id="courses" className="py-20 bg-gray-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <h2 className="text-3xl font-extrabold text-gray-900 mb-12 text-center">Our Courses</h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {courses.map((course, idx) => (
              <div key={idx} className="bg-white border border-gray-200 rounded-lg shadow-sm p-6 flex flex-col justify-between">
                <div>
                  <h3 className="text-xl font-bold text-gray-900 mb-3">{course.title}</h3>
                  <p className="text-gray-700 mb-6">{course.description}</p>
                </div>
                <button className="mt-auto inline-flex justify-center items-center py-2 px-4 text-white font-medium rounded-lg bg-gradient-to-br from-green-400 via-teal-400 to-cyan-500 hover:from-green-500 hover:to-cyan-600 focus:ring-4 focus:ring-green-300">
                  Enroll Now
                </button>
              </div>
            ))}
